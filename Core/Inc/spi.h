...
//#define ENC28J60_RXSIZE		0x1A0E                     // (6670)
//#define ENC28J60_RXSTART	0x0000
//#define ENC28J60_RXEND		(ENC28J60_RXSIZE - 1)      // (6669)
//#define ENC28J60_TXPPCB	   (ENC28J60_RXEND + 1)       // (6670)
//#define ENC28J60_TXSTART	(ENC28J60_TXPPCB + 1)      // (6671)
////#define ENC28J60_TXSTART	(ENC28J60_TXPPCB)      // (6671)
//#define ENC28J60_TXEND		0x1FFF                     // (8191)
...
static const uint16_t ENC28J60_MAXFRAME      = 1514; //!< Maximum frame length minus 4 byte crc appended by hardware
static const uint16_t ENC28J60_TXEND     = 0x1FFF; //!< End of transmit buffer which is end of available memory
static const uint16_t ENC28J60_TXPPCB    = ENC28J60_TXEND - ENC28J60_MAXFRAME - 7; //!< Address of 'per packet control byte', immediately before start of tx Buffer
static const uint16_t ENC28J60_TXSTART   = ENC28J60_TXPPCB + 1; //!< Start of transmit buffer which gives space for 1 packet (1514 frame bytes, 7 Tx status bytes)
static const uint16_t ENC28J60_RXSTART   = 0x0000; //!< Start of recieve circular buffer (erata B7.5 work-around is to place Rx buffer at 0x0000)
static const uint16_t ENC28J60_RXEND     = ENC28J60_TXPPCB - 1; //!< End of recieve circular buffer
static const uint16_t RX_HEADER_SIZE    = 6; //!< Quantity of bytes in the ENC28J60 recieve header, not part of the received packet
static const uint16_t RX_BUFFER_SIZE    = ENC28J60_RXEND - ENC28J60_RXSTART + 1;
...
#define ENC28J60_READ_RX_BUFFER  0x3A
...
public:
   ...
   void onClrSelect(void);
   void onSetReset(void);
   void onClrReset(void);
   void onSetReqTX(void);

   void enc28j60_tx(uint8_t data);
   uint8_t enc28j60_rx(void);
